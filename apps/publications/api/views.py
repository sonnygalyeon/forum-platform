...
from apps.publications.permissions import (
    IsPublicationAuthorOrReadOnly,
)
from apps.publications.selectors import (
...
class PublicationListCreateView(
    ...
    def get_queryset(self):
        ...
        if community_id:
            queryset = queryset.filter(
                community__public_id=community_id
            )

        if author_id:
            queryset = queryset.filter(
                author__public_id=author_id
            )

        if tag:
            queryset = queryset.filter(
                tags__slug__iexact=tag
            )
    ...
    def create(
        ...
        **kwargs,
        ...
        output = PublicationDetailSerializer(
            publication,
            context={
                "request": request,
            },
...
class PublicationRevisionListView(
...
class PublicationRevisionDetailView(
