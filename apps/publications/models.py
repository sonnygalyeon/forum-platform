...
class Tag(models.Model):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(Lower("slug"), name="publications_tag_slug_ci_unique")]
        ordering = ["slug"]
    def __str__(self):
        return self.name
...
class Publication(models.Model):
    ...
    community = models.ForeignKey(Community, on_delete=models.PROTECT, related_name="publications", null=True, blank=True)
    ...
    visibility = models.CharField(max_length=16, choices=Visibility.choices, default=Visibility.PUBLISHED)
...
class PublicationRevision(models.Model):
    publication = models.ForeignKey(Publication, on_delete=models.PROTECT, related_name="revisions")
    revision_number = models.PositiveIntegerField()
    editor = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="publication_revisions")
    title = models.CharField(max_length=300, blank=True)
