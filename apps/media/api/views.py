...
from apps.media.api.serializers import (
    ...
    SignedPartsResponseSerializer,
    UploadCompleteSerializer,
    UploadInitiateSerializer,
    UploadPartNumbersSerializer,
)
...
class UploadInitiateView(UploadBaseView):
    ...
    def post(self, request):
        ...
        if part_count > MAX_MULTIPART_PARTS:
            return Response(
                {"detail": "File requires too many multipart chunks."},
...
class UploadPartsSignView(UploadBaseView):
    ...
    def post(self, request, asset_id):
        ...
        if any(number > asset.part_count for number in part_numbers):
            return Response(
                {"detail": "Part number is outside this upload."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request, asset_id):
        ...
        with transaction.atomic():
            ...
            if actual_numbers != expected_numbers:
                return Response(
                    {"detail": "Completed parts do not match the upload manifest."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            metadata = head_object(object_key=asset.object_key)
            ...
            if actual_size != asset.size_bytes:
                ...
                return Response(
                    {"detail": "Uploaded object size does not match the declared size."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            asset.status = (
                MediaAsset.Status.PENDING_SCAN
                if settings.MEDIA_REQUIRE_SCAN
                else MediaAsset.Status.READY
            )
            asset.upload_id = ""
            asset.completed_at = timezone.now()
            asset.save(update_fields=["status", "upload_id", "completed_at"])
    ...
    def post(self, request, asset_id):
        with transaction.atomic():
            ...
            if asset.status != MediaAsset.Status.UPLOADING:
                return Response(
                    {"detail": "Only active uploads can be aborted."},
                    status=status.HTTP_409_CONFLICT,
                )
