...
def community_queryset_for_user(user):
    queryset = Community.objects.filter(is_active=True).select_related("owner").annotate(subscriber_count=Count("subscriptions", distinct=True))
    if user.is_authenticated:
        ...
        queryset = queryset.annotate(is_subscribed=Value(False, output_field=BooleanField()))
    return queryset
